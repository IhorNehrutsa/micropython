...
def is_motor_running(self):
    ...
    """
    return self.query_motor_status() > MotorStatus.MotorStop.value
...
def wait_for_motor_idle(self, timeout=15):
    ...
    while ((time.time() - start_time < timeout) if timeout else True) and self.is_motor_running():
...
def run_motor_relative_motion_by_pulses(self, direction: Direction, speed, acceleration, pulses):
    ...
    try:
        status = RunMotorResult(status_int)
        if status == RunMotorResult.RunFail:
            raise motor_status_error("run_motor_relative_motion_by_pulses(): RunFail")
        return status
    except ValueError:
...
def run_motor_absolute_motion_by_pulses(self, speed, acceleration, absolute_pulses):
    ...
    try:
        status = RunMotorResult(status_int)
        if status == RunMotorResult.RunFail:
            raise motor_status_error("run_motor_absolute_motion_by_pulses(): RunFail")
        return status
    except ValueError:
        raise motor_status_error(f"No enum member with value {status_int}")
...
def run_motor_relative_motion_by_axis(self, speed, acceleration, relative_axis):
    ...
    try:
        status = RunMotorResult(status_int)
        if status == RunMotorResult.RunFail:
            raise motor_status_error("run_motor_relative_motion_by_axis(): RunFail")
        return status
    except ValueError:
        raise motor_status_error(f"No enum member with value {status_int}")
...
#         raise motor_already_running_error("")
    ...
    try:
        status = RunMotorResult(status_int)
        if status == RunMotorResult.RunFail:
            raise motor_status_error("run_motor_absolute_motion_by_axis(): RunFail")
        return status
    except ValueError:
        raise motor_status_error(f"No enum member with value {status_int}")
