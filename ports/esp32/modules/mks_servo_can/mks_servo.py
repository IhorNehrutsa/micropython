...
#                 self.notifier.add_listener(receive_message)
            #t = time.ticks_ms()
            #print(self.bus)
            #print('send: msg:', msg, 'can_id:', self.can_id)
            self.bus.send(tuple(msg), self.can_id)
            #print('send:', time.ticks_ms() - t)
        except Exception as e:
            raise CanMessageError(f"Error sending message: {e}")

        # Wait for response (with a timeout)
#         start_time = time.time()
#         while (time.time() - start_time < self.timeout) and status is None:
#             time.sleep(0.1)  # Small sleep to prevent busy waiting
#         if self.notifier:
#             self.notifier.remove_listener(receive_message)
        #t = time.ticks_ms()
        recv = self.bus.recv(timeout=10_000)
        #t = time.ticks_ms()
        #print('send - recv:ms:', time.ticks_ms() - start_time)
        if recv:
            message = Message(arbitration_id=recv[0], is_extended_id=recv[1], rtr=recv[2], data=recv[3])
            receive_message(message)
        #print('message', time.ticks_ms() - t)
        return status
